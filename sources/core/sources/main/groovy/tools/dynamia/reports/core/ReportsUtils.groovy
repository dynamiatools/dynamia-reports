...
class ReportsUtils {
    ...
    static EnumFilterProvider findEnumFilterProvider(String className) {
        return Containers.get().findObjects(EnumFilterProvider).find { it.enumClass.name == className }
    }

    static EntityFilterProvider findEntityFilterProvider(String className) {
        return Containers.get().findObjects(EntityFilterProvider).find { it.entityClass.name == className }
    }
